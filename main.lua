...
function love.load()
	...
	keyboard.register_key_binding("menu", "n", function ()
		...
		reset_game()
		action_runner.add_action(function () apply_gravitation() player.move() return game_over_check() end)
	end)
	...
	drawer.register_draw_action("menu", function() deep_sky.draw() love.graphics.print("Press n for a new game, \n! to quit", 100, 100) end)
...
function draw_player()
	...
	love.graphics.line(middle_w + 10*math.cos(player.angle), middle_h + 10*math.sin(player.angle), 
		middle_w + 10*math.cos(player.angle + math.pi*0.9), middle_h + 10*math.sin(player.angle + math.pi*0.9))

	love.graphics.line(middle_w + 10*math.cos(player.angle), middle_h + 10*math.sin(player.angle), 
		middle_w + 10*math.cos(player.angle - math.pi*0.9), middle_h + 10*math.sin(player.angle - math.pi*0.9))

	if player.animation > 0.0001 then
		color_themes.apply_color(color_themes.prothagonist_green, love.graphics.setColor, player.animation)
		love.graphics.line(middle_w + 10*math.cos(player.angle), middle_h + 10*math.sin(player.angle), 
			middle_w + 200*math.cos(player.angle), middle_h + 200*math.sin(player.angle))
		love.graphics.circle("fill", middle_w + 200*math.cos(player.angle), middle_h + 200*math.sin(player.angle), 12)
	end
